...
class RoomService (
    ...
    fun findMatchingRooms(dateFrom: LocalDateTime, dateTo: LocalDateTime, roomStandard: RoomStandard,
                          city: String, nrPeople: Int )
                                                        = roomRepository.findAvailableRoomsForGivenDate(
                                                        dateFrom, dateTo, roomStandard, city, nrPeople
                                                        ).map {toRoomDto(it) }
    fun toRoomDto (room: Room) : RoomDto {
        return RoomDto(room.id, room.price, room.peopleNr, room.description, room.roomStandard.toString())
    }
}